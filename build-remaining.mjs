/**
 * build-remaining.mjs
 * Processes the scraped pages 17-20 markdown file and builds listing pages
 * for any Sundgren listings not yet built.
 */
...
function buildPageHtml(listing) {
  ...
  const heroImg = listing.photoUrl || '/images/hero-house.jpg';
  ...
  const metaDesc = `${listing.beds ? listing.beds + ' bed, ' : ''}${listing.baths ? listing.baths + ' bath' : ''}${listing.sqft ? ', ' + parseInt(listing.sqft).toLocaleString() + ' sq ft' : ''} home${listing.acres && parseFloat(listing.acres) > 0.5 ? ' on ' + listing.acres + ' acres' : ''} at ${listing.address}. ${price}. Listed by ${listing.agent || 'Sundgren'} with Sundgren Realty.`.trim();
...
// Load existing listing IDs from JSON
const existingJson = JSON.parse(fs.readFileSync('./data/sundgren-listings.json', 'utf8'));
const existingIds = new Set(existingJson.map(l => l.listingId));
console.log(`Existing built IDs: ${existingIds.size}`);

// Parse the scraped file
const markdown = fs.readFileSync('./data/https---realestate-sundgren-com-i-residential-prop.txt', 'utf8');
const listings = parseListingsFromMarkdown(markdown);
console.log(`Found ${listings.length} Sundgren listings in pages 17-20`);
...
for (const listing of listings) {
  ...
  if (!listing.address || !listing.listingId) {
    console.log(`SKIP (missing data): listingId=${listing.listingId}`);
    skipped++;
    continue;
  }
...
console.log(`Total listing dirs now: ${fs.readdirSync('./listings').filter(d => fs.statSync('./listings/' + d).isDirectory()).length}`);
