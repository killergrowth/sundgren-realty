...
// Build top cities for city dropdown (any city with 10+ listings)
const cityCounts = {};
listings.forEach(l => {
  const c = cityOf(l).trim().toLowerCase();
    ...
    <div class="container">
      ...
      <div style="display:flex;flex-wrap:wrap;gap:12px;align-items:center;margin-bottom:20px;">
        <div style="position:relative;">
          <select id="city-filter" aria-label="Filter by city" style="appearance:none;-webkit-appearance:none;padding:9px 36px 9px 14px;border:1.5px solid var(--border);border-radius:8px;font-size:13px;font-weight:700;color:var(--dark);background:#fff;cursor:pointer;outline:none;transition:border-color .2s;">
            <option value="">All Cities</option>
            ${topCities.map(c => `<option value="${c}">${titleCase(c)}</option>`).join('\n            ')}
          </select>
          <i class="fas fa-chevron-down" style="position:absolute;right:12px;top:50%;transform:translateY(-50%);color:#aaa;font-size:11px;pointer-events:none;"></i>
        </div>
        <div style="position:relative;">
          <select id="sort-select" aria-label="Sort listings" style="appearance:none;-webkit-appearance:none;padding:9px 36px 9px 14px;border:1.5px solid var(--border);border-radius:8px;font-size:13px;font-weight:700;color:var(--dark);background:#fff;cursor:pointer;outline:none;transition:border-color .2s;">
            <option value="default">Sundgren First</option>
            <option value="price-asc">Price: Low to High</option>
            <option value="price-desc">Price: High to Low</option>
            <option value="newest">Newest First</option>
            <option value="type">Type (Residential / Land)</option>
          </select>
          <i class="fas fa-chevron-down" style="position:absolute;right:12px;top:50%;transform:translateY(-50%);color:#aaa;font-size:11px;pointer-events:none;"></i>
        </div>
        <a href="#" onclick="clearFilters();return false;" id="clear-filters-link" style="font-size:13px;color:var(--text-light);text-decoration:none;display:none;">&#x2715; Clear filters</a>
      </div>
...
document.addEventListener('DOMContentLoaded', function(){
  ...
  function getCardPrice(c) {
    var txt = c.querySelector('.listing-card-price') ? c.querySelector('.listing-card-price').textContent.replace(/[^0-9]/g,'') : '';
    return txt ? parseInt(txt) : 0;
  }
  function getCardDate(c) {
    return c.dataset.listed ? parseInt(c.dataset.listed) : 0;
  }

  function sortCards() {
    var sorted;
    if (activeSort === 'price-asc') {
      sorted = cards.slice().sort(function(a,b){ return getCardPrice(a)-getCardPrice(b); });
    } else if (activeSort === 'price-desc') {
      sorted = cards.slice().sort(function(a,b){ return getCardPrice(b)-getCardPrice(a); });
    } else if (activeSort === 'newest') {
      sorted = cards.slice().sort(function(a,b){ return getCardDate(b)-getCardDate(a); });
    } else if (activeSort === 'type') {
      sorted = cards.slice().sort(function(a,b){ return a.dataset.type.localeCompare(b.dataset.type); });
    } else {
      sorted = originalOrder.slice(); // default: Sundgren first (already DOM order from build)
    }
  ...
  function applyFilters() {
    ...
    cards.forEach(function(c) {
      ...
      var show = matchFilter && matchCity && matchSearch;
  ...
  window.clearFilters = function() {
    ...
    sortCards(); // reset to original order
    applyFilters();
    hideAutocomplete();
  };
